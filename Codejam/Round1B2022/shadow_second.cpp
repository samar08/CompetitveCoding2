...
void testcase(){
	...
	dp[0][1] = 0;
	for(int i = 1;i <= n;i++){
		dp[i][0] = min(dp[i-1][1] + abs(a[i-1][m-1]-a[i][m-1]),dp[i-1][0] + abs(a[i-1][0]-a[i][m-1])) + (a[i][m-1] - a[i][0]);
		dp[i][1] = min(dp[i-1][1] + abs(a[i-1][m-1]-a[i][0]),dp[i-1][0] + abs(a[i-1][0]-a[i][0])) + (a[i][m-1] - a[i][0]);
	}
